...
% The original MATLAB script is publicly published on Github as an open source resource
% Credit to the people at KAIST (Korea Advanced Insitute of Science and Technology)
% Modified by Hoang Trung Le for the fulfilment of UTS Capstone project
...
for i = 1:simulationTime/0.01
   ...
   figure(2)
   subplot(2,3,1);
   plot(i/100,drone1_state(7)*R2D,'.'); % phi
   subplot(2,3,2);
   plot(i/100,drone1_state(8)*R2D,'.'); % tta
   subplot(2,3,3);
   plot(i/100,drone1_state(9)*R2D,'.'); % psi
   subplot(2,3,4);
   plot(i/100,drone1_state(1),'.'); % x
   subplot(2,3,5);
   plot(i/100,drone1_state(2),'.'); % y
   subplot(2,3,6);
   plot(i/100,drone1_state(6),'.'); % dz
